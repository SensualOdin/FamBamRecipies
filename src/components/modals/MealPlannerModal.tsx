            ...
            <div className="flex justify-between items-center mb-8">
              <div className="flex items-center gap-3">
                <Button
                  ...
                  size="icon"
                  onClick={onClose}
                  className="w-10 h-10 bg-slate-50 dark:bg-white/5 hover:bg-slate-100 dark:hover:bg-white/10 rounded-xl border border-slate-100 dark:border-white/10 shrink-0"
                >
              ...
              <div className="hidden sm:flex items-center gap-2 bg-slate-50 dark:bg-white/5 p-1 rounded-2xl">
                ...
                <span className="px-4 font-black text-sm uppercase tracking-tighter text-slate-700 dark:text-slate-300 min-w-[120px] text-center">{currentMonth.toLocaleString('default', { month: 'long', year: 'numeric' })}</span>
              ...
              {days.map(day => {
                ...
                return (
                  <div
                    ...
                    `}
                  >
                    <span className={`text-sm font-black ${isToday && !isSelected ? 'text-detroit-600 dark:text-detroit-400' : isSelected ? 'text-white' : 'text-slate-600 dark:text-slate-400'}`}>{day}</span>
                    {meals.length > 0 && (
            ...
            {selectedDate ? (
              <div className="flex flex-col h-full animate-fadeIn">
                <div className="flex justify-between items-start mb-8">
                  ...
                  {!showRecipePicker && (
                    <button onClick={() => setShowRecipePicker(true)} className="w-10 h-10 bg-slate-900 dark:bg-detroit-500 text-white rounded-xl flex items-center justify-center shadow-lg hover:scale-110 transition-all"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 4v16m8-8H4" strokeWidth={3} /></svg></button>
                  )}
                </div>
                ...
                <div className="flex-1 overflow-y-auto scrollbar-hide -mx-4 px-4">
                  {showRecipePicker ? (
                    <div className="space-y-3">
                      ...
                      {recipes.map(r => (
                        <button key={r.id} onClick={() => handleAddMeal(r)} className="w-full group p-4 bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-white/10 hover:border-detroit-200 dark:hover:border-detroit-500/30 transition-all flex items-center gap-4 text-left">
                          <div className="w-12 h-12 rounded-xl overflow-hidden bg-slate-50 dark:bg-slate-700 flex items-center justify-center shrink-0">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-tighter">{r.category}</p>
                          </div>
                        </button>
                      ))}
                    </div>
                    ...
                    <div className="space-y-4">
                      {(mealPlan[selectedDate] || []).length === 0 ? (
                        <div className="text-center py-12">
                          <div className="w-16 h-16 bg-white dark:bg-slate-700 rounded-3xl flex items-center justify-center mx-auto mb-4 text-3xl shadow-sm">🥘</div>
                          <p className="text-muted-foreground text-sm font-medium">Nothing planned yet.<br/>What's for dinner?</p>
                        </div>
                      ) : (
                        mealPlan[selectedDate].map((m, i) => (
                          <div key={i} className="bg-white dark:bg-slate-800 rounded-[32px] p-6 shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-white/10 relative group">
                            ...
                            <div className="flex items-center gap-4 mb-6">
                              <div className="w-14 h-14 rounded-2xl overflow-hidden bg-slate-50 dark:bg-slate-700 flex items-center justify-center shrink-0">
                              ...
                              <div className="flex-1 min-w-0">
                                <h4 className="font-black text-foreground leading-tight">{m.title}</h4>
                                <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest mt-1">{m.cookTime}</p>
                              </div>
                            </div>
                            <button onClick={() => setShowCalendarExport({ meal: m, date: selectedDate })} className="w-full py-3 bg-slate-50 dark:bg-white/5 hover:bg-slate-100 dark:hover:bg-white/10 rounded-2xl text-[10px] font-black uppercase tracking-widest text-slate-600 dark:text-slate-400 transition-all flex items-center justify-center gap-2">
              ...
              <div className="flex flex-col items-center justify-center h-full text-center py-12">
                <div className="w-20 h-20 bg-white dark:bg-slate-700 rounded-[32px] flex items-center justify-center mb-6 text-4xl shadow-sm">📅</div>
                <h3 className="text-lg font-bold text-foreground mb-2 tracking-tight">Select a Date</h3>
                <p className="text-muted-foreground text-sm max-w-[200px]">Choose a day from the calendar to start planning your family's meals.</p>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-[60] flex items-center justify-center p-6 animate-fadeIn" onClick={() => setShowCalendarExport(null)}>
            <div className="bg-white dark:bg-slate-800 rounded-[40px] p-10 max-w-sm w-full animate-scaleIn shadow-2xl" onClick={e => e.stopPropagation()}>
              <div className="text-center mb-8">
                ...
                <h3 className="text-2xl font-black text-foreground tracking-tight mb-2">Sync to Calendar</h3>
              ...
              <div className="space-y-3">
                <button onClick={() => { window.open(getGoogleCalendarUrl(showCalendarExport.meal, showCalendarExport.date), '_blank'); setShowCalendarExport(null); }} className="w-full flex items-center gap-4 p-4 bg-slate-50 dark:bg-white/5 rounded-[24px] hover:bg-slate-100 dark:hover:bg-white/10 transition-all group">
                  <div className="w-10 h-10 bg-white dark:bg-slate-700 rounded-xl flex items-center justify-center shadow-sm text-xl">G</div>
                  <span className="flex-1 font-bold text-slate-700 dark:text-slate-300 text-left">Google Calendar</span>
                  <svg className="w-5 h-5 text-slate-300 group-hover:text-slate-900 dark:group-hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 5l7 7-7 7" strokeWidth={3} /></svg>
                </button>
                <button onClick={() => downloadICS(showCalendarExport.meal, showCalendarExport.date)} className="w-full flex items-center gap-4 p-4 bg-slate-50 dark:bg-white/5 rounded-[24px] hover:bg-slate-100 dark:hover:bg-white/10 transition-all group">
                  <div className="w-10 h-10 bg-white dark:bg-slate-700 rounded-xl flex items-center justify-center shadow-sm text-xl">📅</div>
                  <span className="flex-1 font-bold text-slate-700 dark:text-slate-300 text-left">Apple / Outlook</span>
                  <svg className="w-5 h-5 text-slate-300 group-hover:text-slate-900 dark:group-hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 5l7 7-7 7" strokeWidth={3} /></svg>
                </button>
              </div>

              <button onClick={() => setShowCalendarExport(null)} className="w-full mt-8 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors">Cancel</button>
            </div>
          </div>
        )}
      </div>
