...
import { 
  Heart, Plus, ShoppingCart, Edit2, 
  Clock, Users, ChefHat, Flame, Star
} from 'lucide-react';
...
const RecipeCard = ({ recipe, index, onClick, onToggleFavorite, onAddToShoppingList, onAuthorClick, onEdit }) => {
  ...
  return (
    <motion.div
      ...
      className="group relative flex flex-col h-full bg-white rounded-[32px] overflow-hidden transition-shadow duration-300 border border-slate-100/50 hover:shadow-[0_20px_50px_rgba(0,0,0,0.1)]"
    >
      ...
      <div className="p-6 flex flex-col flex-1">
        ...
        <div className="flex items-center justify-between gap-3 pt-2">
          ...
          <div className="flex items-center gap-2 flex-1 justify-end">
            ...
            <motion.button 
              whileTap={{ scale: 0.95 }}
              className="flex items-center gap-2 bg-slate-900 text-white px-5 py-2.5 rounded-2xl font-bold text-xs hover:bg-slate-800 transition-all shadow-lg shadow-slate-900/10"
            >
              Cook Now
            </motion.button>
          </div>
        ...
        {recipe.timesCooked > 0 && (
          <motion.div 
            ...
            className="absolute top-1/2 left-0 right-0 -translate-y-1/2 flex justify-center pointer-events-none z-30 opacity-0"
          >
